    ...
    <section className="mx-auto w-full max-w-3xl">
      <div className="relative overflow-hidden rounded-2xl border border-zinc-800 bg-[#101012]/90 shadow-[0_30px_90px_rgba(0,0,0,0.4)] before:pointer-events-none before:absolute before:inset-x-8 before:top-0 before:h-px before:bg-gradient-to-r before:from-transparent before:via-zinc-500 before:to-transparent">
        <div className="flex items-center justify-between border-b border-zinc-800 bg-zinc-900/30 px-6 py-3 text-xs sm:px-8">
          <span className="font-medium text-zinc-400">GitGlow resume comparison</span>
        ...
        <div className="p-6 sm:p-8">
          <div className="flex flex-col justify-between gap-7 border-b border-zinc-800 pb-8 sm:flex-row sm:items-center">
            <div>
              <p className="text-xs font-medium uppercase tracking-[0.14em] text-zinc-500">Resume vs GitHub · @{report.username}</p>
              <h1 className="mt-3 text-3xl font-semibold tracking-[-0.055em] text-zinc-100">Resume alignment score</h1>
              ...
              <div className="mt-4 overflow-hidden rounded-xl border border-zinc-800">
                <table className="w-full text-left text-sm">
                  ...
                  <tbody className="divide-y divide-zinc-800">
                    {analysis.skillGaps.map((gap: { skill: string; status: string; evidence: string }) => (
                      <tr key={gap.skill} className="bg-[#111113]">
              ...
              <ul className="mt-4 space-y-2">
                {analysis.experienceGaps.map((gap: string) => (
                  <li key={gap} className="flex items-start gap-3 text-sm leading-5 text-zinc-400">
                    <span className="mt-1.5 size-1 rounded-full bg-[#d7ff54]" />
                    {gap}
                  </li>
                ))}
              </ul>
      ...
      <div className="mt-7 flex flex-col items-center justify-center gap-3 sm:flex-row">
        <Link href="/resume" className="inline-flex items-center gap-2 rounded-full px-4 py-2 text-sm font-medium text-zinc-400 transition hover:text-[#d7ff54]">
